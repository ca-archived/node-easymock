...
MockServer.prototype.readFileJson = function(file, options) {
  ...
  data = data.replace(TEMPLATE_PATTERN, function(match) {
    ...
    return JSON.stringify(self.readFileJson(templateFile, {args: args, params: options.params} ));
  });
...
MockServer.prototype.getApiDocumentation = function(callback) {
  ...
  walk(folder, function(err, results) {
    ...
    results = _.map(results, function(file) {
      ...
      var response = that.readFileJson(requestInfo.file, {params: requestInfo.params});
      return {
        method: method,
        path: path,
...
MockServer.prototype.generateApiDocumentation = function(callback) {
  ...
  var pathCss  = __dirname + '/res/documentation.css';
  var pathJs  = __dirname + '/res/documentation.js';
  var pathZeptoJs  = __dirname + '/res/zepto.min.js';
  ...
  function writeDocumentationHtml(html) {
    ...
    var pathCssOut = folder + '/documentation.css';
    var pathJsOut = folder + '/documentation.js';
    var pathZeptoJsOut = folder + '/zepto.min.js';
    fs.mkdir(folder, function() {
      fs.writeFile(pathHtmlOut, html, function (err) {
        if (err) { return callback(err); }
        // TODO refactor this!
        fs.createReadStream(pathCss).pipe(fs.createWriteStream(pathCssOut));
        fs.createReadStream(pathJs).pipe(fs.createWriteStream(pathJsOut));
        fs.createReadStream(pathZeptoJs).pipe(fs.createWriteStream(pathZeptoJsOut));
        callback(null);
      });
    });
  }
